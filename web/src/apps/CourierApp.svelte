<script>
  ...
  import {
    ...
    courierToken,
    courierLogout,
  } from '../lib/state/courierSession.svelte.js';
  import CourierLoginScreen from '../lib/screens/courier/CourierLoginScreen.svelte';
  import CourierHomeScreen from '../lib/screens/courier/CourierHomeScreen.svelte';
...
{:else if !logged}
  <CourierLoginScreen onLoggedIn={() => (logged = true)} onApply={() => (applying = true)} />
{:else}
  <div class="app">
    <header class="top">
      <BrandMark size={30} />
    ...
    <main>
      ...
      {:else if ride}
        ...
        />
      {:else if tab === 'home'}
        <CourierHomeScreen {me} onRefresh={pull} onOpenSettle={() => (settling = true)} />
        {#if me?.shift}
